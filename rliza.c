...
rliza_t *rliza_object_from_string(char **content) {
    ...
    if (**content == '{') {
        ...
        (*content)++;
        char *result = NULL;
        while ((result = rliza_seek_string(content, "\"|,|}|object")) != NULL &&
               *result) {
            if (**content == ',') {
                (*content)++;
                continue;
        ...
        while ((result = rliza_seek_string(
        ...
        *content += 5;
        ...
        if (is_number) {
            ...
            rliza->content.number = strtod(extracted, NULL);
            free(extracted);
        } else {
...
unsigned char *rliza_object_to_string(rliza_t *rliza) {
    ...
    if (rliza->type == RLIZA_INTEGER) {
        ...
        if (rliza->key) {
            sprintf(content, "\"%s\":\"%s\"", rliza->key,
                    rliza->content.string);
        } else {
            sprintf(content, "\"%s\"", rliza->content.string);
        }
    } else if (rliza->type == RLIZA_NUMBER) {
        if (rliza->key) {
            sprintf(content, "\"%s\":\"%f\"", rliza->key,
        ...
        for (unsigned i = 0; i < rliza->count; i++) {
            strcat(content, rliza_object_to_string(rliza->content.map[i]));
            strcat(content, ",");
        }
